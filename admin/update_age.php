...
<?php
$id=$_REQUEST['id'];
$sql="select * from age_group where id='$id'";
$result=mysqli_query($con,$sql);
$row=mysqli_fetch_array($result);
?>
<form method="post">
<label>Age Group</label>
<input type="text" name="age" value="<?php echo $row['age'];?>" required class="form-control mb-2">
...
if(isset($_POST['submit'])){
	$age=$_POST['age'];
	$sql="update age_group set admin_id='".$admin['id']."', age='$age' where id='$id'";
	$result=mysqli_query($con,$sql);
	if($result){
		echo "<script>window.location.href='age.php'
		alert('Age Group Updated Successfully')</script>";
	}
	else{
		echo "<script>window.location.href='age.php'
